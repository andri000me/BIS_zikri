...
function select_ar(){
   $query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_dokumen ");
    // "SELECT k.*,concat((SELECT TIMESTAMPDIFF(MONTH,CURDATE(),periode_akhir)),' Bulan') AS sisa_sewa FROM kantor_cabang AS k INNER JOIN tbl_user AS tu ON k.id_regional = tu.kode_reg WHERE tu.id = ".$_SESSION['id']." AND k.id_regional = tu.kode_reg");
...
function select_cabang_reg(){
    $query = mysqli_query($GLOBALS["___mysqli_ston"],"SELECT k.*,concat((SELECT TIMESTAMPDIFF(MONTH,CURDATE(),periode_akhir)),' Bulan') AS sisa_sewa FROM kantor_cabang AS k INNER JOIN tbl_user AS tu ON k.id_regional = tu.kode_reg WHERE tu.id = ".$_SESSION['id']." AND k.id_regional = tu.kode_reg");
...
function select_pbb(){
      $query=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM kantor_cabang WHERE status_pbb = 1 ");
    return $query;
}
...
function select_email($id)
{
  $send_email=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT
        ...
        FROM
            ...
            join regional b on b.id = a.id_regional
        WHERE
            a.status_kepemilikan = '2' AND a.status in ('1','2') AND a.id = '$id'
            AND ( SELECT TIMESTAMPDIFF( MONTH, CURDATE( ), a.periode_akhir ) ) <=12");
  return $send_email;
}
...
//     }
// }
// function setButtonStatusCaption($status)
// {
//     if($status=='0')
//     {
//         return "OFF";
